...
docker service update --with-registry-auth --image $IMAGE django_app || \
docker service create --with-registry-auth --name django_app --mode global --publish 8000:8000 --constraint 'node.role==worker' $IMAGE
